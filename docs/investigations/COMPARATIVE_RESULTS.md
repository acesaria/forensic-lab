...
| ubuntu-22.04_ptrace_fa_20260807-150736 | ptrace_fa (vanilla) | Userland / `ptrace` foreign-allocation shellcode injection | `4/0/0/0`; `4/4` (100.0%) | `3/0/0/0`; `3/3` (100.0%) | `5/0/0/0`; `5/5` (100.0%) | `8/0/0/0`; `8/8` (100.0%) | `U/C/S: 0/3/5`; `X: 0`; gain `+3` (P01, P03, P04) | FS `N/A`; TL `N/A`; Mem `3`; union `3` | FS `43s`; TL `80s`; Mem not measured | FS: TSK 4.15.0; TL: Plaso 20260512 `psort`; Mem: Volatility 3 2.28.0 |
...
For the `ptrace_fa` case, the small 8-target inventory is fully exposed:
filesystem and timeline together describe staging/build (P01–P04, read from
the same acquired disk through two tools — corroboration, not two independent
acquisitions), while memory alone recovers every runtime target (P05–P08). The
`+3` union gain is drawn entirely from targets outside memory's applicability
(the injector process had exited by acquisition, and P01/P04 are static/staged
facts), not from a target an applicable source tried for and missed; the case
summary states this explicitly. Filesystem and timeline `TTF` were recorded
immediately at their first accepted result; memory's marker was recorded only
after several later plugin invocations and is therefore reported as not
